  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        const { user } = await listenToAuthResult(createUserWithEmailAndPassword(auth, formData.email, formData.password));
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-6 bg-base">
      <div className="w-full max-w-sm rounded-[32px] p-8 pb-12 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center mb-8">
          ...
          <p className="text-text-secondary text-sm mt-1">Unlock your creativity</p>
        </div>

        <div className="relative z-10 bg-surface/50 p-1 rounded-2xl mb-8 flex border border-white/5 backdrop-blur-md">
          <button
        ...
        <form onSubmit={handleSubmit} className="relative z-10 flex flex-col gap-4">
              ...
              >
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-surface border ${errors.name ? 'border-red-500/50' : 'border-white/10'} rounded-[16px] py-3.5 pl-11 pr-4 text-sm focus:outline-none focus:border-primary-blue/50 focus:ring-1 focus:ring-primary-blue/50 transition-all`}
                  />
                </div>
                {errors.name && <span className="text-red-400 text-xs pl-2">{errors.name}</span>}
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="flex flex-col gap-1">
            <div className="relative">
              ...
              <input
                ...
                onChange={handleChange}
                className={`w-full bg-surface border ${errors.email ? 'border-red-500/50' : 'border-white/10'} rounded-[16px] py-3.5 pl-11 pr-4 text-sm focus:outline-none focus:border-primary-blue/50 focus:ring-1 focus:ring-primary-blue/50 transition-all`}
              />
            </div>
            {errors.email && <span className="text-red-400 text-xs pl-2">{errors.email}</span>}
          </div>
          ...
          <div className="flex flex-col gap-1">
            <div className="relative">
              ...
              <input
                ...
                onChange={handleChange}
                className={`w-full bg-surface border ${errors.password ? 'border-red-500/50' : 'border-white/10'} rounded-[16px] py-3.5 pl-11 pr-12 text-sm focus:outline-none focus:border-primary-blue/50 focus:ring-1 focus:ring-primary-blue/50 transition-all`}
              />
              <button
                type="button"
            ...
            {errors.password && <span className="text-red-400 text-xs pl-2">{errors.password}</span>}
          </div>
              ...
              >
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className={`w-full bg-surface border ${errors.confirmPassword ? 'border-red-500/50' : 'border-white/10'} rounded-[16px] py-3.5 pl-11 pr-4 text-sm focus:outline-none focus:border-primary-blue/50 focus:ring-1 focus:ring-primary-blue/50 transition-all`}
                  />
                </div>
                {errors.confirmPassword && <span className="text-red-400 text-xs pl-2">{errors.confirmPassword}</span>}
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {isLogin && (
            <div className="flex justify-end mt-[-8px]">
              <button type="button" className="text-xs text-primary-blue hover:text-primary-blue/80 font-medium">
                Forgot password?
              </button>
            </div>
          )}
        ...
        <div className="relative z-10 mt-8 mb-4">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-base px-4 text-text-muted">Or continue with</span>
          </div>
        </div>
        ...
        <div className="relative z-10 flex gap-4">
          <motion.button
            ...
            type="button"
            disabled={loading}
            className="flex-1 bg-surface border border-white/10 rounded-xl py-3.5 flex items-center justify-center hover:bg-elevated transition-colors"
          >
            {/* Google SVG */}
